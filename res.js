...
// @version      0.20220408185031
// @description  在电视上听使用网易云,目前只能听歌……
// @author       You
...
// @updateURL https://markedboat.com/cors/js_file?file=tampermonkey/_release/singly/music.163.com/res.js
// @downloadURL  https://markedboat.com/cors/js_file?file=tampermonkey/_release/singly/music.163.com/res.js
// ==/UserScript==
...
(function () {
    ...
    if (XMLHttpRequest.prototype.realOpen3 === undefined) {//网易云自己也改写了个realOpen……  重名了
...
// };
        ...
        timerCfg.triggers.forEach(function (trigger, index) {
            ...
            if (trigger.isFlagOpen() && timerCfg.counts % trigger.mod === 0) {
                ...
                if (trigger.end > -1 && trigger.end < timerCfg.counts) {
                    //console.log('outer:',);
                    trigger.closeFlag();
                    trigger.outFun(trigger);
                } else {
                    //console.log('in:',);
                    trigger.inFun(trigger);
                }
            }
        });
    }, 100);
    ...
    try {
        ...
        console.log('tampermonkey/_release/singly/music.163.com/src/index.js');
        ...
        (function () {
                ...
                top.window.kl.kiwiJs.showMsg = function (msg, ttl) {
                    ...
                    if (msg_box.auto_close_st > 0) {
                        window.clearTimeout(msg_box.auto_close_st);
                        msg_box.auto_close_st = 0;
                    }
                    msg_box.classList.remove('hide');
                    if (typeof ttl === 'number' && ttl > 0) {
                        msg_box.auto_close_st = window.setTimeout(function () {
                            msg_box.auto_close_st = 0;
                        ...
                        if (top && top.document && top.document.body) {
                            ...
                            if (!top.document.getElementById('kl_kiwi_menu_style')) {
                                ...
                                top.document.body.append(new Emt('style', 'id="kl_kiwi_menu_style"', '' +
                                    '#kl_kiwi_menu_root{position: fixed; background: #EEE;width: 100%; height: ' + h + 'px; top: 0px; left: 0px; z-index: 99999; font-size: 20px;}' +
                                    ...
                                    '.kl_kiwi_menu_root_msg_box{position: fixed;width: 100%;height: 100%;top: 0;left: 0;z-index: 999999;' +
                                    'background: #666;\n' +
        ...
        console.log('tampermonkey/_release/singly/music.163.com/src/website.js');
        ...
        (function () {
                    ...
                    top.window.kl.kiwiJs.captureInfo = {
                        music163: {
                            ...
                            loadLyric: function (text) {
                                console.log('default_loadLyric', text);
                            },
                            loadAudio: function (link) {
                                console.log('default_loadAudio', link);
                            },
                            timer: {
                                triggerMap: [],
                                triggers: [],
                ...
                addTimerTrigger({
                    ...
                    in_fun: function (trigger) {
                        ...
                        if (top && top.document && top.document.body) {
                            ...
                            if (document.location.href.indexOf('https://music.163.com/login') === 0) {
                                if (kl.xpathSearch('.//*[(contains(text(),"扫码登录"))]').length > 0) {
                                    top.window.kl.log('看样可以直接扫码');
                                } else {
                                    let tmp_checkboxs = kl.xpathSearch('.//input[@type="checkbox" and @id="j-official-terms"]');
                                    if (tmp_checkboxs.length === 1) {
                            ...
                            if (document.location.href === 'https://music.163.com/#' || document.location.href === 'https://music.163.com/' || document.location.href === 'https://music.163.com') {
                                if (kl.xpathSearch('.//*[(contains(text(),"我的主页"))]').length === 0) {
                                    if (kl.xpathSearch('.//a[@data-action="login"]').length === 0) {
                                        console.log('xxxxx', kl.xpathSearch('.//*[(contains(text(),"我的主页"))]'));
                ...
                let lyricPlayer = function (lyric_str) {
                    ...
                    self.loadLyric = function (lyric_str) {
                        ...
                        self.catchLyricBoxSizeTimer.setNewTtl(3, 0, function (trigger) {
                            trigger.closeFlag();
                            console.log('catch_lyric_box_size_timer');
                    ...
                    console.log('addTimerTrigger', addTimerTrigger);
                ...
                if (document.location.href.indexOf('music/playlist?id=') > 0) {
                    console.log('playlist 页面逻辑 start');
                    ...
                    let play_btn_click_timer = addTimerTrigger({
                        ...
                        in_fun: function (trigger) {
                            trigger.closeFlag();
                            console.log('play_btn_click_timer');
                        },
                        out_fun: function () {
                        }
                    });
                    let catch_comment_timer = addTimerTrigger({
                        ...
                        in_fun: function (trigger) {
                            trigger.closeFlag();
                            console.log('catch_comment_timer');
                        },
                        out_fun: function () {
                        }
                    });
                    ...
                    songs_div.addNodes([new Emt('iframe', 'class="hide"').setIndexHandler(songs_div, 'ifr')]);
                    let player_div = new Emt('div', 'class="kl_kiwi_play_box hide"');//歌词显示、audio element、播放控制 的 root div
                    let comment_div = new Emt('div', 'class="kl_kiwi_play_box hide kl_kiwi_comment_box"');//评论
                    player_div.songId = 0;
                    ...
                    let flush_lyric_timer = addTimerTrigger({
                        ...
                        in_fun: function (trigger) {
                            //console.log('xxxxxx', kl.xpathSearch('.//table[contains(@class,"m-table")]//span[@data-res-action="play"]', document.getElementById('g_iframe').contentDocument).length);
                            if (audio_ele.paused) {
                                return false;
                            }
                            if (lyric_div.player) {
                    ...
                    player_div.resetSongInfo = function () {
                    ...
                    catch_comment_timer.setNewTtl(-1, -1, function (trigger) {
                        let cs = kl.xpathSearch('.//div[@class="m-cmmt"]//div[@class="itm"]', songs_div.ifr.contentDocument);
                        if (cs.length > 0) {
                            trigger.closeFlag();
                            let tmp_ar = [];
                            let tmp_size = 5;
                            let tmp_len = Math.ceil(cs.length / tmp_size);
                            for (let tmp_j = 0; tmp_j < tmp_len; tmp_j++) {
                                let tmp_div = new Emt('div', 'class="comment_list_div"');
                                let tmp_start = tmp_j * tmp_size;
                                let tmp_ar2 = cs.slice(tmp_start, tmp_start + tmp_size);
                                tmp_ar2.forEach(function (tmp_ele) {
                                    tmp_div.addNode(tmp_ele);
                                });
                                tmp_ar.push({
                                    tagName: 'div',
                                    attrStr: 'class="comment_div"',
                                    tabIndex: tmp_j,
                                    appendElement: tmp_div,
                            ...
                            comment_grid.loadSourceArray(tmp_ar).loadGridData2UI(false);

                        }
                    });
                    songs_div.ifr.addEventListener('load', function () {
                    ...
                    songs_div.loadNewSongSrc = function (audio_link) {
                        ...
                        audio_ele.play();
                        audio_grid.song_title.textContent = player_div.sourceBtn.init_data.text;
                    };
                    ...
                    switch_grid.waitingCaptureSongInfo = function (raw_btn, cell_emt) {
                        console.log('waitingCaptureSongInfo', cell_emt, cell_emt.init_data);
                        if (player_div.songId === cell_emt.init_data.songId) {
                            player_div.classList.remove('hide');
                            audio_grid.focusUILastGridCell('songs_grid');
                            return false;
                        }
                        player_div.songId = cell_emt.init_data.songId;
                        songs_div.ifr.src = cell_emt.init_data.link;

                        player_div.classList.remove('hide');
                        audio_grid.focusUILastGridCell('init');

                        player_div.resetSongInfo();
                        ...
                        top.window.kl.kiwiJs.showMsg('正在加载歌曲信息->' + cell_emt.init_data.text);
                        //  console.clear();
                        top.window.kl.log('清理 captureSongInfoInIframe lyric & audio_link clean');
                        player_div.sourceBtn = cell_emt;
                                ...
                                if (1 || info_index < 100) {
                                    song_btns.push({
                                        ...
                                        fun: function (cell_emt) {
                                            ...
                                            console.log(play_btns, './/span[@data-res-action="play" and @data-res-id="' + info.id + '"]');
                    ...
                    addTimerTrigger({
                        ...
                        in_fun: function (trigger) {
                            ...
                            if (window.innerHeight > 0) {
                                ...
                                let avg_w = Math.floor(window.innerWidth / songs_grid.config.ui.cols);
                                let avg_h = Math.floor(window.innerHeight / (songs_grid.config.ui.rows + 0.5));

                                let comment_avg_w = Math.floor(window.innerWidth / comment_grid.config.ui.cols);
                                let comment_avg_h = Math.floor(window.innerHeight / (comment_grid.config.ui.rows));
                                ...
                                top.document.body.append(new Emt('style', 'id="kl_kiwi_menu_style_website"', '' +
                                    ...
                                    '.hide23:focus{border:1em solid #F00;}' +
                                    ...
                                    '.comment_div{with:100%;height:100%;border:2px solid #000;}' +
                                    ...
                                    '.kl_kiwi_comment_box td{display:block;width:' + comment_avg_w + 'px;height:' + comment_avg_h + 'px;}' +

                                    '.kl_kiwi_play_box{}' +
                                    '.kl_kiwi_play_ctrl_box button{background:#FFF !important;border:3px solid #FFF !important;color:#000 !important;}' +
                                    '.kl_kiwi_play_ctrl_box button:focus{border:solid #000 3px !important;background:#FFF !important;color:#000 !important;}' +


                                    '.kl_kiwi_list_box{background: #EEE;width: 100%; min-height:100%;max-height:100%; }' +
                                    '.kl_kiwi_list_box_title{ background: #EEE;width: 100%; height: 10%; top: 0px; font-size: 20px;}' +
                                    '.kl_kiwi_list_box_list{ background: #EEE;width: 100%;height:90%;  top: 0px; font-size: 14px;}' +
                                    '.kl_kiwi_list_box_list>table{ display:block;float:left;width: 100%; height:100%;}' +
                                    '.kl_kiwi_list_box_list>table tr{ width: 100%; }' +
                                    '.kl_kiwi_list_box_list>table tr td{ display:block;float:left; }' +

                                    '.kl_kiwi_list_box_fixed{position: fixed; top: 0px; left: 0px; z-index: 999999;background: #EEE;width: 100%;max-height:100%; }' +
